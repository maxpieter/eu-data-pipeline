...
export default function RebelScoresPage() {
  ...
  useEffect(() => {
    async function loadConfig() {
      try {
        const response = await fetch('/rebel-score/data/config.json')
    ...
    async function loadData() {
      ...
      try {
        ...
        const allMepResponse = await fetch(
          `/rebel-score/data/periods/${currentPeriod}/mep_data.json`
        )
        ...
        let url: string
        if (currentTopic === 'all') {
          url = `/rebel-score/data/periods/${currentPeriod}/mep_data.json`
        } else {
          const slug = configRef.topics[currentTopic]
          url = `/rebel-score/data/periods/${currentPeriod}/topics/${slug}.json`
        }
